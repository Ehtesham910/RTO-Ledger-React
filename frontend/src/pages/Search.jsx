...
function Search() {
    ...
    const totalResults = results.customers.length + results.vehicles.length + results.serviceRequests.length + results.receipts.length;
        ...
        <div className="page-container">
            <div className="page-header">
                <div>
                    ...
                    <p className="page-subtitle">Found {totalResults} result(s) across all records</p>
                </div>
            </div>
            ...
            {results.customers.length > 0 && (
                <div style={{ marginBottom: '2rem' }}>
                    <h3 style={{ marginBottom: '1rem', color: '#1e293b' }}>Customers ({results.customers.length})</h3>
                        ...
                        <table className="data-table">
                            ...
                            <tbody>
                                {results.customers.map(c => (
                                    <tr key={c.id}>
                                        <td><span className="badge">{c.customer_code}</span></td>
                                        <td className="font-medium">{c.name}</td>
                                        <td>{c.mobile}</td>
                                        <td>
                                            <button className="btn-action view" onClick={() => navigate('/customers')}>View</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                            ...
                            <tbody>
                                {results.vehicles.map(v => (
                                    <tr key={v.id}>
                                        ...
                                        <td>
                                            <button className="btn-action view" onClick={() => navigate('/vehicles')}>View</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                            ...
                            <tbody>
                                {results.serviceRequests.map(r => (
                                    <tr key={r.id}>
                                        ...
                                        <td>
                                            <button className="btn-action view" onClick={() => navigate('/services/requests')}>View</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                            ...
                            <tbody>
                                {results.receipts.map(r => (
                                    <tr key={r.id}>
                                        ...
                                        <td>{r.ledgers?.customers?.name || '-'}</td>
                                        <td>₹{r.amount_received}</td>
                                        <td>
                                            <button className="btn-action view" onClick={() => navigate(`/receipts/${r.id}`)}>View</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
