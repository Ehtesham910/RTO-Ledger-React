...
function Ledger() {
    ...
    const handleDeleteLedger = (ledgerId, customerName) => {
        if (window.confirm(`Are you sure you want to delete ledger record for "${customerName || 'this customer'}"? This will also remove any associated receipts.`)) {
            axios.delete(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/ledger/${ledgerId}`)
                .then(() => {
                    const updatedList = ledgers.filter(item => item.id !== ledgerId);
                    setLedgers(updatedList);
                    sessionStorage.setItem('ledgerData', JSON.stringify(updatedList));
                })
                .catch((error) => {
                    console.error("Error deleting ledger record:", error);
                    alert("Failed to delete ledger record.");
                });
        }
    };
