...
const demoLogin = (req, res) => {
  ...
  const token = jwt.sign(
    { id: "demo", username: "demo", role: "Demo" },
    JWT_SECRET,
    { expiresIn: "24h" },
  );
  ...
  res.json({
    token,
    user: { id: "demo", username: "demo", role: "Demo", permissions },
  });
};
  ...
  try {
    ...
    if (customer.password && customer.password.startsWith("$2")) {
      ...
      isMatch =
        ...
        (!customer.password && password === "customer123");
    }
    ...
    res
      .status(500)
      .json({ error: "Internal server error during customer login" });
  }
};
...
const customerRegister = async (req, res) => {
  try {
    ...
    if (!name || !mobile || !password) {
      return res
        .status(400)
        .json({ error: "Name, mobile, and password are required" });
    }
